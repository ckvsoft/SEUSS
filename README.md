...
| `web_socket_url`    | Optional fully-qualified WebSocket URL for the live updates on the status and stats pages. Leave empty for the default direct-port behaviour (same hostname as the page, port `8765`). Set this when SEUSS sits behind a reverse proxy / TLS terminator that doesn't forward port 8765 — the proxy needs to route the URL you put here to the SEUSS WebSocket server (port 8765 internally). Example: `"wss://seuss.example.com/ws"` if your proxy maps `/ws` to `host:8765` with WebSocket upgrade enabled.   |

...
class PowerConsumptionManager:
    def __init__(self, initial_config=None):
        ...
        self.logger.log_debug(f"initial config is {initial_config}")
    ...
    def update_instance(self, new_config):
        ...
        self.logger.log_debug(f"new config is {new_config}")
        if not self.current_instance:  # If the instance has not been initialized yet
            self.logger.log_debug("No instance available, trying to initialize...")
        ...
        if self.current_config is None:
            self.logger.log_debug("current_config is None, initializing instance...")
            self.initialize_instance(new_config)
        ...
        if new_config["type"] != self.current_config["type"]:
            self.logger.log_debug(f"Type changed from {self.current_config['type']} to {new_config['type']}. Updating instance...")
            if self.current_instance:
                self.current_instance.stop()  # Stop the current instance
                self.current_instance = None
            self.initialize_instance(new_config)
            self.logger.log_debug("Instance successfully updated.")
        else:
            # The type remains the same, so only update the configuration if necessary
            self.logger.log_debug("The type remains the same. Checking if configuration update is needed...")
            if new_config != self.current_config:
                self.logger.log_debug("Updating the configuration...")
                self.current_instance.update_config(new_config)
                self.logger.log_debug("Instance successfully updated.")
            else:
                self.logger.log_debug("Configuration update not needed...")
